...
<a href="add_storage_form.php">Add Storage</a>
<div class="row">
    ...
    <div class="col-md-6 mb-4">
        <div class="storage-card p-3 border rounded">
            ...
            <div class="storage-metrics">
                <p><strong>Temp:</strong> <?= htmlspecialchars($storage['current_temp']) ?>°C</p>
                <p><strong>Humidity:</strong> <?= htmlspecialchars($storage['humidity']) ?>%</p>
                <p><strong>Capacity:</strong> <?= htmlspecialchars($storage['used_capacity']) ?>/<?= htmlspecialchars($storage['total_capacity']) ?>kg</p>
                <a href="edit_storage_form.php?id=<?= $storage['id'] ?>" class="btn btn-sm btn-primary">Edit</a>
                <a href="delete_storage.php?id=<?= $storage['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
            </div>
        </div>
    </div>
<?php endforeach; ?>
...
<script>
document.addEventListener('DOMContentLoaded', function () {
    // Enhance each card with a data-location attribute (using its h5 content)
